// src/demo.ts
// ─────────────────────────────────────────────
// DEMO MODE — runs the full agent workflow with
// simulated/mock responses so you can see the
// complete flow without real API keys.
// ─────────────────────────────────────────────
...
async function demo() {
  ...
  // ─────────── STEP 2: SAP DISCOVERY ───────────
  logger.info("🔍 DEMO", "=== STEP 2: SAP TOOL DISCOVERY ===");
  logger.info("🔍 DEMO", "Querying OOBE Synapse SAP Registry...");
  await sleep(500);

  const tools = await discoverTools(task); // Uses fallback list (no real API needed)
...
`.trim();
  ...
  logger.info("💳 DEMO", "Step 1: POST /api/v1/orders/{order_id}/pay/ → 402 Payment Required");
  await sleep(400);
  logger.info("💳 DEMO", "Received payment requirement: 0.0025 USDC → Base mainnet");
  await sleep(300);
  ...
  console.log(`  Tx Hash    : 0x7f3a2b1c...demo`);
  console.log("\x1b[35m╚══════════════════════════════════════════════════════════╝\x1b[0m\n");

  console.log("\x1b[36m💡 To run with real APIs, copy .env.example → .env and fill in your keys.\x1b[0m\n");
}
